...
class LandingController extends Controller
{
    ...
    public function Doregister(Request $request)
    {
        ...
        if ($request->p_no != "" && substr($request->p_no, 0, 2) == "83") {
         ...
         $userEmailcheck=DB::table('register')->where('email',$request->email)->first();
         $userPhonecheck=DB::table('register')->where('p_no',$request->p_no)->first();
        if($userEmailcheck){
            Session::flash("message", "Email Alredy Exit's");
            return redirect("register");
        }
        if($userPhonecheck){
            Session::flash("message", "Number Alredy Exit's");
            return redirect("register");
        }
